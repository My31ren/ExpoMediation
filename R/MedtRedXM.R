...
#' @param Method Method used for mediator reduction wth all exposures, available options are "mean"(default) and "pdm1"
...
#' @examples eSet <- MedtRedXM(eSet = eSet,
#'                   Method = "mean",
#'                   Iter = 500)
MedtRedXM = function(eSet,
                     Method = "mean",
                     Iter = 500){
  #iter: the iterations of mediation modellling
  tictoc::tic()
  lubridate::now() %>%
    stringr::str_replace_all(":",".") %>%
    stringr::str_replace_all("-",".") -> NowTime

  path = stringr::str_c(eSet$FileDirOut, "/XMreduction")
  ...
  MReduction = function(
    data = eSet$Expo$Data,
    Y = "Y1",
    X = eSet$Expo$Data[eSet$Expo$Voca %>%
    ...
    F3 = function(M){
      F2 <- function(m, m_grpname){
        ...
        ddpcr::quiet(tmp3 <- purrr::map_dfr(X, F1))
        return(tmp3)
      ...
      if (tibble::is_tibble(M)==TRUE) {
        ...
        m <- as.matrix(M)
        eSet$MedtRedXM$all <- F2(m, m_grpname)
        eSet$MedtRedXM$all %>%
          vroom::vroom_write(stringr::str_c(path, "/MedtRedXM-all-mediators-",NowTime, ".csv"), ",")
        #message and log
        message(stringr::str_c("Complete XMreduction for all mediators!", "\n", "The users may search eSet$MedtRedXM$all or see XMreduction folder for detailed results.", "\n", NowTime), "\n")
        eSet$ExecutionLog <- eSet$AddLog(stringr::str_c("Complete XMreduction for all exposures!", NowTime))
      }
      else if (tibble::is_tibble(M)==FALSE) {
        m_grpname = rlist::list.names(M)
        names(M) <- NULL
        m <- purrr::map(M, as.matrix)
        eSet$MedtRedXM$list <- purrr::map2_dfr(m, m_grpname, F2)
        eSet$MedtRedXM$list %>%
          vroom::vroom_write(stringr::str_c(path, "/MedtRedXM-each-mediator-group-",NowTime, ".csv"), ",")
        #message and log
        message(stringr::str_c("Complete XMreduction for each mediator group!", "\n", "The users may search eSet$MedtRedXM$list or see Mreduction folder for detailed result.", "\n", NowTime), "\n")
        eSet$ExecutionLog <- eSet$AddLog(stringr::str_c("Complete XMreduction for each mediator group!", NowTime))
      }
      return(eSet)
    }#end F3
  ...
  eSet <- MReduction(
    data = tmp,
    Y = "Y1",
    X = c(colnames(eSet$MedtRedX$ERSall),colnames(eSet$MedtRedX$ERSlist)),
    C = eSet$Expo$Voca %>%
      dplyr::filter(stringr::str_detect(SerialNo, "^C")) %>%
      dplyr::select(SerialNo) %>%
      as.matrix() %>%
  ...
  eSet$RCommandLog <- eSet$AddCommand(stringr::str_c("eSet <- MedtRedXM(eSet, Iter = Iter)"))
  eSet$RCommandLog %>%
    tibble::as_tibble() %>%
